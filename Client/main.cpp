...
int main() {
    ...
    sf::RenderWindow materialWindow(sf::VideoMode(sf::Vector2u(600, 750)), "Materials");
    ...
    while (window.isOpen())
    {
        ...
        if (checkNewData()) {
            ...
            if (command == "removeWall") {
                std::string tag;
                data >> tag;
                std::cout << "Removing wall with tag: " << tag << "\n";
...
#define acc 10
        ...
        //===============
        {
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::D) && veiwCanvas.mouseOver && canDelWall)
            {
                ...
                {
                    std::string tag = results[veiwCanvas.getMouseOverPixel(window).x].hits[0].wall->tag;
